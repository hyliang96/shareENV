...
1931-1932年，薛定谔提出一个思想实验，N个布朗运动的粒子随时间演化，N很大，至少在阿伏伽德罗常数规模。观测到的经验分布，初始分布为$\rho_0$，最终分布为$\rho_1$。然而，按照布朗运动从$\rho_0$演化到$t=1$理论上应得到的最终分布，却和$\rho_1$不一致。

找一个尽可能像布朗运动的轨迹分布，使得初始分布演化成最终分布。
