...
def inference(a):
    ...
    with torch.no_grad():
        ...
        if len(traj) > 0:
            y_traj, src_traj, ftr_traj = traj[0], traj[1], traj[2]
            print('y_traj.shape', y_traj.shape)
            ...
            for i, t in enumerate(sample_steps):
                ...
                save_audio(net_v(y_traj[:, t, ...]), out)
                src_out = os.path.join(a.output_dir, f'{src_name}_to_{trg_name}_src_t={t}.wav')
                save_audio(net_v(src_traj[:, t, ...]), src_out)
                ftr_out = os.path.join(a.output_dir, f'{src_name}_to_{trg_name}_ftr_t={t}.wav')
                save_audio(net_v(ftr_traj[:, t, ...]), ftr_out)
...
def main():
    ...
    a = parser.parse_args()
    config = os.path.join('configs/config.json')
