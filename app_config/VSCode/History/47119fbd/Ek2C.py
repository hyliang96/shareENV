...
'''
This script is used to remove the unmaintained attachments and empty directories in Zotfile/Attanger destination dir.
from: https://gist.github.com/hyliang96/48df61a12cff6abf93d88ae8150c704e
'''
...
def remove_unmaintained(attachments_to_remove):
    '''
    Remove the unmaintained files in PosixPath type
    '''
    [p.unlink() for p in attachments_to_remove]

def get_empty_dirs():
    '''
    Get a list of empty dirs in PosixPath type
    '''
    empty_dirs = [
    ...
    return [Path(p) for p in empty_dirs]

def remove_empty_dirs(empty_dirs):
    '''
    Remove the empty dirs in PosixPath type
    '''
    [shutil.rmtree(p.as_posix(), ignore_errors=True) for p in empty_dirs]


def click_remove(remove_func, remove_list, prompt_str):
    print('\nThe following '+str(len(remove_list))+' '+prompt_str+':')
    if len(remove_list):
        print('\n'.join(['  {}'.format(p) for p in remove_list]))
        try:
            import click
...
if __name__ == '__main__':
    ...
    click_remove(remove_unmaintained, attachments_to_remove, 'files are no longer managed by Zotero')

    empty_dirs = get_empty_dirs()
    click_remove(remove_empty_dirs, empty_dirs, 'empty directories')
