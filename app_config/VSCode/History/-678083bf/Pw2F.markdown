...
$$
  x_t = a_t x_s +  b_t z, z\sim N(0,I),\\
  a_t:= \exp \int_s^t f(\tau)\dd \tau\\
  c_t:=\left(\int_s^t \frac{g^2(\tau)}{a_{\tau|s}^2}\dd \tau\right)^{1/2}\\
  b_t:=a_t c_t
$$
...
* 当$g(t)\not\equiv 0$，使用此性质，原前向SDE变为
  ...
  故通解为
  $$
  \frac{x_t}{a_t}-\frac{x_s}{a_{s|s}}=c_t z, z\sim N(0,I),\\
  c_t^2:=\int_0^t \frac{g^2(\tau)}{a_{\tau|s}^2}\dd \tau, c_t>0
  $$
  利用性质$a_{s|s}=1$，化简即得通解为
  $$
  x_t=a_tx_s+a_tc_tz , z\sim N(0,I)
  $$
* 若已知$a_t,b_t$，想反向求$f,g$：
