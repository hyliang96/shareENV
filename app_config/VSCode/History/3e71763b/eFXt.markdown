...
* 为降低维度，只考虑该矩阵的对角线元素，取$\sigma_t=\tr(\Sigma)/d, d=\dim(x)$

1、跟**x**txt无关：为了消除对**x**txt的依赖，我们对全体**x**txt求平均，即**Σ**t**=**𝔼**x**t**∼**p**(**x**t**)**[**Σ**(**x**t**)**]**Σt=Ext∼p(xt)[Σ(xt)]；

2、单位阵的倍数：这意味着我们只用考虑对角线部分，并且对对角线元素取平均，即**σ**¯**2**t**=**Tr**(**Σ**t**)**/**dσ¯t2=Tr(Σt)/d，其中**d**=**dim**(**x**)d=dim⁡(x)。
