...
* 为避免训练从$x_t$预测方差矩阵的网络（会因为方差矩阵维度太高而学不好，拉低diffusion表现），需要使方差和$x_t$无关，故求真实协方差矩阵$\Sigma=\mathbb{E}_{x_t\sim q(x_t)} \Sigma(x_t)$，

* 为降低维度，只考虑该矩阵的对角线元素，取$\sigma_t=\dfrac{\tr(\Sigma)}{d}, d=\dim(x)$
...
$$
\begin{align}
\Sigma&=\mathbb{E}_{x_t\sim q(x_t)} \Sigma(x_t)\\
...
&=\frac{b_t^2}{a_t^2}\mathbb{E}_{x_0\sim q(x_0)}\mathbb{E}_{z_t\sim N(0,I)}[\frac{1}{b^2_t}(x_t-a_t x_0)(x_t-a_t x_0)^\top-\epsilon_\theta(x_t,t)\epsilon_\theta(x_t,t)^\top]\\
\end{align}
$$