...
因为$\Phi^\alpha\Z$是P-局部鞅，故存在$\tau_k,k\in \N$，使得$\forall s<\tau_k,\forall t\in[s,\tau_k]$，有$\mathbb E^P[\Phi_t^\alpha Z_t|\mathcal F_s]=\Phi_s^\alpha Z_s$

$\forall s<\tau_k,\forall t\in[s,\tau_k], \forall A\in \mathcal F_s,\forall \alpha\in\R$，有
...
$$
\mathbb E^Q[Y^*_t - [Y]_t|\mathcal F_s]=Y^*_s - [Y]_s
$$
