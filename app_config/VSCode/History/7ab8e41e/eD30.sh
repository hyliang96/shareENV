...
for file in "${zhihu_html_dir}"/pins/*.html; do
    ...
    perl -i -pe 'BEGIN{undef $/;} s|\{\.(origin_image\|content_image).+?\}||smg' "${file_out}"
    <a data-first-child="" target="_blank" href="https://www.zhihu.com/zvideo/1385654193277952000?utm_source=ZHShareTargetIDMore"
