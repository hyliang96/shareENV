...
zhihu_backup_dir="$here/../backup"
# zhihu_user_dir="${zhihu_backup_dir}/${user_id}"
markdown_dir="${zhihu_user_dir}/markdown"

# # rm "${zhihu_user_dir}/record"

mv "${zhihu_user_dir}" "${zhihu_user_dir}-$(date '+%Y-%m-%d-%H-%M-%S')"
mkdir -p "${zhihu_backup_dir}"
mkdir "${zhihu_user_dir}"
# cd "${zhihu_user_dir}"; https_proxy='' http_proxy='' python -c "from zhihubackup import backup_zhihu; backup_zhihu('${user_id}')"
...
# n_total=$(ls -1 "${zhihu_user_dir}"/**/*.html | wc -l)

# n=0

# for file in "${zhihu_user_dir}"/**/*.html; do

#     n=$(( 1 + $n ))

#     relative_file="$(realpath --relative-to="${zhihu_user_dir}" "${file}")"
