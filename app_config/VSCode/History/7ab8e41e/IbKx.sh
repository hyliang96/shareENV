...
for file in "${zhihu_html_dir}"/${testtype}/${testfile}.html; do
    ...
    perl -i -pe 's|(https\:\/\/vdn\.vzuu\d*\.com\/[A-Za-z0-9.?_=&\/\-]+)|<iframe src="\1" allowfullscreen="allowfullscreen" width="100%" height="600" scrolling="no" frameborder="0" sandbox="allow-top-navigation allow-same-origin allow-forms allow-scripts"> </iframe>|g' "${file_out}"
...
for file_out_dir in "${markdown_dir}"/${testtype}/${testfile}.textbundle; do
    ...
    for i in $(cat "${file_out}" | grep -E '\<iframe src="https\:\/\/vdn\.vzuu\d*\.com\/[A-Za-z0-9]+\/[0-9a-z\-]+\.mp4[A-Za-z0-9\/\-.?_=&]+"' | perl -pe 's|\<iframe src="(https\:\/\/vdn\.vzuu\d*\.com\/[A-Za-z0-9]+\/[0-9a-z\-]+\.mp4[A-Za-z0-9\/\-.?_=&]+)|\1|g'); do
        filename="${i##*/}"
        filename="${filename%%\?*}"
        echo $filename
        # wget --no-check-certificate -c -t 0 "$i" -O "${file_out_dir}/assets/$filename"
