...
class Diffusion_SB_VP(BaseModule):
    ...
    # f(t)=-beta_t/2
    # g^2(t)=beta(t) = beta_0 + (beta_1 - beta_0) * t
    # beta_0 = beta_min, beta_1 = beta_max
    # t=1时, sigma2_1 = (g2_max+g2_min)/2
    # cul_beta_t = int_0^t beta(s) ds = beta_0 * t + (beta_1 - beta_0) * t^2 / 2
    # alpha_t=exp(-1/2 cul_beta_t)
    # sigma^2_t=exp(cul_beta_t)-1
    ...
    def forward_diffusion(self, x0, mask, x1, t):
        ...
        if self.predictor == "hpsi":
            ...
            # 式(24) hat_Psi_t: eps_hat_psi_t = (xt-alpha_t*x1)/(alpha_t*sigma_t)
            ...
            # 式(24) Psi_t: eps_psi_t = (xt-bar_alpha_t*x1)/(alpha_t*bar_sigma_t), bar_alpha_t=alpha_1/alpha_t
    ...
    def data_estimation(self, xt, mask, x1, t, spk):
        ...
        if self.predictor == "hpsi":
            ...
            # 式(24): Psi_t: x_t = bar_alpha_t*x1 + eps_psi * alpha_t * bar_sigma_t, eps_psi ~ N(0,1)
            # xt_input 即 eps_psi
            # 但为什么要把 eps_psi 输入到 estimator，去估计 eps_hat_psi ？
            eps_t = self.estimator(xt_input, mask, normalized_x1, t, spk)

            hat_x0 = xt/alpha_t - sigma_t * eps_t
            # 式(24): Psi: xt = alpha_t*x0 + eps_hat_psi * alpha_t * sigma_t, eps_hat_psi ~ N(0,1)
            #   变形得： x0 = xt/alpha_t - sigma_t * eps_hat_psi
            # 也即式(36)变形得: 估计值 hat_x0 = xt/alpha_t - sigma_t * epsilon_theta^{hat_phi}
