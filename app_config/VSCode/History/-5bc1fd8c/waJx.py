...
class Diffusion_SB_VP(BaseModule):
    # vp: Bridge-VP: variance perserving
    ...
    def __init__(self, n_feats=80, dim=64,
        ...
        # total_cul_beta = get_cul_beta(1) = int_0^1 beta(s) ds
    ...
    def get_bar_alpha(self, t):
        # bar_alpha_t = alpha_1 *  alpha_t = exp(-1/2 * (int_t^1 beta(s) ds) ) = alpha_1 *  alpha_t = exp(-1/2 * (int_0^1 beta(s) ds - int_0^t beta(s) ds) )
