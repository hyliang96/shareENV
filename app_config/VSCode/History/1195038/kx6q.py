...
def get_empty_dirs():
    '''
    Get a list of empty dirs in string type
    '''
    empty_dirs = [
...
def remove_empty_dirs(empty_dirs):
    '''
    Remove the empty dirs in string type
    '''
...
if __name__ == '__main__':
    ...
    # print('\nThe following '+str(len(attachments_to_remove))+' files are no longer managed by Zotero:')
    ...
    click_remove(remove_empty_dirs, empty_dirs, 'directories are no longer managed by Zotero')

    print('\nThe following '+str(len(empty_dirs))+' empty directories are no longer managed by Zotero:')
    if len(empty_dirs):
        print('\n'.join(['  {}'.format(p) for p in empty_dirs]))
        try:
            import click
            if click.confirm('Do you want remove them?', default=True):
