...
\\
逆向扩散/生成时噪声&=&\boldsymbol z&=\boldsymbol\epsilon\\
逆向扩散/生成时噪声的方差&=&\sigma_t&=\sigma_t
\end{align*}
$$
...
| 方法         | 前传的原始设定                                                                          | 前传$p(x_t\vert x_{t-1})$是否确定 | 反传/采样公式，分布$q(x_{x_-\Delta t}\vert x_t,x_0)$                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | <span style="display:inline-block;width:140px">反传过程是否随机 |
| ---------- | -------------------------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
...
\begin{align}
\forall s>t\geq 0,&\\
q(x_t|x_s,x_0)&=N(\frac{\frac{a_t}{a_s}c_t^2}{c_s^2}x_s+\frac{a_t(c_s^2-c_t^2)}{c_s^2}x_0, \frac{a_t^2c_t^2(c_s^2-c_t^2)}{c_s^2}I)\label{eq:ddpm-x0}\\
&=N(a_{ts}\frac{ b_t^2}{b_s^2}x_s + a_t\frac{b_{ts}^2}{b_s^2}x_0, \frac{b_{ts}^2b_t^2}{b_s^2}I)\\
估计值\hat x_0&=\frac{x_s-b_s\epsilon_\theta(x_s,s)}{a_s}\\
代入估计值，化简为,&\\
q(x_t|x_s,x_0)\approx p(x_t|x_s)&=\mathcal N\left(\frac{1}{a_{ts}}(x_s-\frac{b_{ts}^2}{b_s^2}b_s \epsilon_\theta(x_s,s)) ,\frac{b_{t}^2b_{ts}^2}{b_s^2} I\right)
\end{align}
$$
**DDPM论文里的记号：**
...
>
> 故重新表示$x_t=a_tx_0+b_t z_t=a'_t (a_{t-1}x_0+b_{t-1}z_{t-1})+b'_t z'_t, x_{t-1}$, 则$\mathbb{E}_{x_t,z'_t}$变为$\mathbb{E}_{x_{t-1},z'_t}$再变为$\mathbb{E}_{z_{t-1},z'_t}$。因此，损失函数变为$\dfrac{{b'_t}^2}{{a’_t}^2}\mathbb{E}_{z_{t-1},z'_t} \Vert \epsilon'_\theta(x_t(z_{t-1},z'_t),t)-z'_t \Vert^2$。
>
...
$$
正向SDE \xrightarrow{\text { 推导 }} q\left(\boldsymbol{x}_t \mid \boldsymbol{x}_s\right) \xrightarrow[贝叶斯]{\text { 推导 }}
q\left(\boldsymbol{x}_t \mid \boldsymbol{x}_s, \boldsymbol{x}_0\right)
\xrightarrow[用\frac{x_s-b_s\epsilon_\theta(x_s,s)}{a_s}替代x_0]{\text { 近似 }} p\left(\boldsymbol{x}_{t} \mid \boldsymbol{x}_s\right)
$$
根据[正向直达概率$q_{t|s}$<=正向SDE](../推导diffusion的SDE和ODE.textbundle/text.md#正向直达概率$q_{t%20s}$<=正向SDE)：若扩散过程服从正向SDE，
$$
\dd x_t = f(t)x_t+g(t)\dd w_t, w_t 是标准布朗运动
$$
则直达概率$q(x_t|x_s), \forall t>s\geq 0$为
$$
\begin{gather}
q(\boldsymbol x_t|\boldsymbol x_s)=\mathcal N(\boldsymbol x_t|a_{st} \boldsymbol x_0, b^2_{st} I), \forall  1\geq t\geq s\geq 0,\\
a_{st}:=\exp \int_s^t f(\tau)\dd \tau,\quad b_{st}:=a_{st}c_{st},\quad c^2_{st}:=\int_s^t\frac{g^2(\tau)}{a_{s\tau}^2}\dd \tau\\
\\
a_{st}=\frac{a_t}{a_s},\quad
b^2_{st}=a^2_t(c^2_t-c^2_s)\quad  c_{st}^2=a^2_s(c^2_t-c^2_s)\\
a_t:=a_{0t}= \exp \int_0^t f(\tau)\dd \tau,\quad b_t:=b_{0t}\,\quad  c_t^2:=c_{0t}^2=\int_0^t \frac{g^2(\tau)}{a_{\tau}^2}\dd \tau \\
q(\boldsymbol x_t|\boldsymbol x_0)=\mathcal N(\boldsymbol x_t|a_{t} \boldsymbol x_0, b^2_{t} I), \forall  1\geq t\geq 0
\end{gather}
$$
使用贝叶斯公式，有$\forall s>t\geq 0,$
$$
\begin{gather}
q(x_t|x_s,x_0)=\frac{q(x_t,x_s|x_0)}{q(x_s|x_0)}=\frac{q(x_s|x_t)q(x_t|x_0)}{q(x_s|x_0)}\\
=C_1\exp \left\{-\frac 1 2\left[ \frac{\Vert x_s-a_{ts}x_t\Vert^2_2}{a_s^2(c_s^2-c_t^2)} +  \frac{\Vert x_t-a_{t}x_0\Vert^2_2}{a_t^2c_t^2}-  \frac{\Vert x_s-a_{s}x_0\Vert^2_2}{a_s^2 c_s^2} \right]\right\}\\
=C_1\exp \left\{-\frac 1 2\left[ x_t^2\left(\frac{1}{a_t^2(c_s^2-c_t^2)}+\frac{1}{a_t^2 c_t^2}\right) - 2x_t\left(\frac{x_t}{a_s a_t(c_s^2-c_t^2)}+\frac{ x_0}{a_t c_t^2}\right)+C_2\right]\right\}\\
=\mathcal N(x_t|\mu,\sigma^2 I)
\end{gather}
$$
其中，
$$
\begin{gather}
\sigma^2=1/\left(\frac{1}{a_t^2(c_s^2-c_t^2)}+\frac{1}{a_t^2 c_t^2}\right)=\frac{a_t^2c_t^2(c_s^2-c_t^2)}{c_s^2}\\
\mu=\left(\frac{x_t}{a_s a_t(c_s^2-c_t^2)}+\frac{ x_0}{a_t c_t^2}\right)\sigma^2=\frac{\frac{a_t}{a_s}c_t^2x_s+a_t(c_s^2-c_t^2)x_0}{c_s^2}
\end{gather}
$$
而$x_0$可以使用下面的方法进行单点估计：
$$
\begin{gather}
x_s=a_s x_0+b_s z_s, z_s\sim \mathcal N(0,I),\\
\epsilon_\theta(x_s,s)\approx z_s\\
\therefore x_0\approx \hat x_0:=\frac{x_s-b_s \epsilon_\theta(x_s,s)}{a_s}\\
其中b_s=a_s c_s
\end{gather}
$$
将其代入$\mu$，得
$$
\begin{align}
\mu&=\frac{1}{c_s^2}\left[\frac{a_t}{a_s}c_t^2x_s+a_t(c_s^2-c_t^2)\frac{x_s-b_s \epsilon_\theta(x_s,s)}{a_s}\right]\\
&=\frac{a_t}{a_s}\frac{1}{c_s^2}\left[c_t^2x_s+(c_s^2-c_t^2)(x_s-b_s \epsilon_\theta(x_s,s)\right]\\
&=\frac{a_t}{a_s}\frac{1}{c_s^2}\left[c_s^2x_s-(c_s^2-c_t^2)b_s \epsilon_\theta(x_s,s)\right]\\
&=\frac{a_t}{a_s}\frac{1}{c_s^2}\left[c_s^2x_s-\frac{b_{ts}^2}{a_s^2}b_s \epsilon_\theta(x_s,s)\right]  //由于b_{ts}^2=a_s^2(c_s^2-c_t^2)\\
&=\frac{1}{a_{ts}}\left[x_s-\frac{b_{ts}^2}{b_s} \epsilon_\theta(x_s,s)\right] //由于a_{ts}=\frac{a_s}{a_t}, b_s=a_s c_s
\end{align}
$$
且方差还能化简为
$$
\sigma^2=\frac{a_t^2c_t^2(c_s^2-c_t^2)}{c_s^2}=\frac{b_t^2 \frac{b_{ts}^2}{a_s^2}}{c_s^2}=\frac{b_{t}^2b_{ts}^2}{b_s^2}
$$
故
$$
q(x_t|x_s,x_0)\approx p(x_t|x_s)=\mathcal N\left(\frac{1}{a_{ts}}(x_s-\frac{b_{ts}^2}{b_s^2}b_s \epsilon_\theta(x_s,s)) ,\frac{b_{t}^2b_{ts}^2}{b_s^2} I\right)\label{eq:ddpm-st}
$$
#### 使用DDPM原文的记号
...
\xrightarrow[用\frac{x_t-b_t\epsilon_\theta(x_t,t)}{a_t}替代x_0]{\text { 近似 }} p\left(\boldsymbol{x}_{t-1} \mid \boldsymbol{x}_t\right)\label{ddpm-framework}
$$
...
$$
q(x_{t-1}|x_t)=N(x_{t-1}|\dfrac{1}{a'_t}\big( x_t - \frac{{b'_t}^2}{b_t}\epsilon_\theta(x_t,t)\big),(\dfrac{b_{t-1}}{b_t}b'_t )^2 I)
$$
...
$$
上式和st记号推导出的结果式$\eqref{eq:ddpm-st}$的结果一致，只需赋值$(s,t)\leftarrow (t,t-1),a_{ts}\leftarrow a'_t,b_{ts}\leftarrow b'_t$即可证明。
...
在st记号下 （在别处记作$s>t\geq 0$，此处记作$t>\tau\geq 0$）：
$$
\begin{gather}
\forall t>\tau\geq 0,\\
x_\tau=\frac{a_\tau c_\tau}{a_t c_t}x_t+a_\tau(1- \frac{c_\tau}{c_t}) x_0\\
即x_\tau=\frac{b_\tau}{b_t}x_t+(a_\tau-a_t \frac{b_\tau}{b_t}) x_0\\
代入估计值\hat x_0=\frac{1}{a_t}(x_t-b_t\epsilon_\theta(x_t,t))，化简得\\
x_\tau=\frac{a_\tau}{a_t}x_t-(\frac{a_\tau}{a_t}b_t-b_\tau)\epsilon_\theta(x_t,t) \label{eq:ddim-st}
\end{gather}
$$
...
$$
q\left(\boldsymbol{x}_{\tau} \mid \boldsymbol{x}_t, \boldsymbol{x}_0\right)=\mathcal{N}\left(\boldsymbol{x}_{\tau} ; \kappa_{t\tau} \boldsymbol{x}_t+\lambda_{t\tau} \boldsymbol{x}_0, \sigma_{t\tau}^2 \boldsymbol{I}\right)
$$
...
x_t&=a_tx_0+b_tz_t\\
&将上两式带入下式\\
x_{\tau}&=\kappa_{t\tau} x_t+\lambda_{t\tau}x_0+\sigma_{t\tau} z
\end{align}
$$
...
$$
a_{\tau}=\kappa_{t\tau} a_t+\lambda_{t\tau} \\
b_{\tau}^2=\sigma_{t\tau}^2+\kappa_{t\tau}^2 b^2
$$

两个方程，求解$\kappa_{t\tau},\lambda_{t\tau},\sigma_{t\tau}$三个变量，解有无穷个，可见$q(\boldsymbol x_{t-1}|\boldsymbol x_t,\boldsymbol x_0)$有很多种选择。故先将$\sigma_t$当作待定参数，去求余下两个变量。

$$
\kappa_{t\tau}=\frac{\sqrt{b^2_{\tau}-\sigma_{t\tau}^2}}{b_t}\\
\lambda_{t\tau}=a_{\tau}-a_t\kappa_{t\tau}
$$

故得到 **$q(x_\tau|x_t,x_0)$重参数化** 表示为

$$
x_\tau=\kappa_{t\tau} x_t+(a_\tau-a_t \kappa_{t\tau})x_0+\sigma_{t\tau} z, z\sim N(0,I)\\
\kappa_{t\tau}=\frac{\sqrt{b^2_{\tau}-\sigma_{t\tau}^2}}{b_t} \label{ddim-repara}
$$

* 当$\sigma_{t\tau}=0$，**为DDIM**，上式变为

  $$
  x_\tau=\frac{b_\tau}{b_t}x_t+(a_\tau-a_t \frac{b_\tau}{b_t})x_0
  $$
* 当$\sigma_{t\tau}=\tilde b'_t$，$\tau=t-1$，为DDPM，上式变为$\eqref{qxt-1-xt-x0}$，即
...
$$
\begin{align}
x_\tau &=\kappa_{t\tau} x_t +(a_\tau-a_t\kappa_{t\tau})\frac{1}{a_t}(x_t-b_t\epsilon_\theta(x_t,t))+\sigma_{t\tau} z_t\\
&=\frac{a_\tau}{a_t}x_t-(\frac{a_\tau}{a_t}b_t-b_t\kappa_{t\tau})\epsilon_\theta(x_t,t)+\sigma_{t\tau} z\\
&=\frac{a_\tau}{a_t}x_t-(\frac{a_\tau}{a_t}b_t-\sqrt{b^2_\tau-\sigma_t^2})\epsilon_\theta(x_t,t)+\sigma_{t\tau} z\label{ddim-general}
\end{align}
$$
...
$$
x_\tau=\frac{\sqrt{\bar\alpha_{\tau}}}{\sqrt{\bar\alpha_t}}(x_t-\sqrt{1-\bar\alpha_t}\epsilon_\theta(x_t,t))+\sqrt{{1-\bar\alpha_{\tau}-\sigma_t^2}}\epsilon_\theta(x_t,t)+\sigma_{t\tau} z,z\sim N(0,I)
$$

* 当$\sigma_{t\tau}=0$，**即为DDIM**，$q(x_t|x_{t-1})$不满足马尔可夫性，式$\eqref{ddim-general}$变为
  ...
  * 若又$\tau=t-1$，则
  ...
  $$
* 当$\sigma_{t\tau}=\tilde b_{t|\tau}:=\dfrac{b_\tau}{b_t}b_{t|\tau}$，为DDPM跳步采样，$q(x_t|x_{t-1})$满足马尔可夫性，式$\eqref{ddim-general}$变为

  $$
  x_\tau=\frac{a_\tau}{a_t} x_t-\frac{b_{t|\tau}^2}{b_t}\epsilon_\theta(x_t,t)+\frac{b_\tau}{b_t}b_{t|\tau}z,z\sim N(0,I)
  $$

  * 若又$\tau=t-1$，即DDPM（默认是单步采样），
    $$
...
思路简介:
* 求出逆向SDE/ODE的精确的积分形式解，
* 用$\epsilon_\theta$估计$z-t$(DPM-solver)/用$x_\theta$估计$x_0$(DPM-solver++)，从而估计$\nabla\log q(x_t)$，
* 再对$\epsilon_\theta$/$x_\theta$进行$k-1$阶Tayler展开，以化简包含$\epsilon_\theta$/$x_\theta$的积分，得到SDE/ODE-DPM-solver(-/++)k。

引自：
* DPM-solver++提出自: Lu, C., Zhou, Y., Bao, F., Chen, J., Li, C., & Zhu, J. (2023). DPM-Solver++: Fast Solver for Guided Sampling of Diffusion Probabilistic Models (No. arXiv:2211.01095). arXiv. https://doi.org/10.48550/arXiv.2211.01095
* Chen, Z., He, G., Zheng, K., Tan, X., & Zhu, J. (2023). *Schrodinger Bridges Beat Diffusion Models on Text-to-Speech Synthesis* (No. arXiv:2312.03491). arXiv. https://doi.org/10.48550/arXiv.2312.03491  section 3.3, proposition 2.

下面推导使用st记号。

## 扩散方程的积分形式解通式

若扩散过程服从正向SDE（$w_t$是标准布朗运动）/ODE（$g(t)\equiv 0$）/反向SDE（$w_{t}$时间反转后是标准布朗运动，通常记为$\bar w_t$）：
$$
\dd {\boldsymbol x_t}=[\phi(t)\boldsymbol x_t+\boldsymbol \psi(\boldsymbol x_t,t)]\dd t+g(t)\dd{\boldsymbol w_t},\\
\boldsymbol x_t, \boldsymbol \psi(\boldsymbol x_t,t), \boldsymbol w_t \in\R^n\label{eq:orig}
$$
其naive的积分形式的解是，
$$
x_t=x_s+\int_s^t[\phi(\tau)x_\tau+\psi(x_\tau,\tau)]\dd \tau+\sqrt {\xi \int_s^t g^2(\tau)\dd \tau} z, z\sim N(0,I)
$$
其中符号为
$$
\xi=\begin{cases}1, &正向SDE ,&  t\geq s\\ 0, &ODE, & t \mathrel{\substack{> \\ = \\ <}} s\\ -1,& 逆向SDE,&t\leq s \end{cases}
$$
更精确的积分形式的解是，
$$
x_t=A_{st} x_s+\int_s^t A_{\tau t} \psi(x_\tau,\tau)\dd \tau+B_{st} z, z\sim N(0,I)\label{eq:sol}
$$
其中，
$$
A_{st}:=e^{\int_s^t \phi(\tau)\dd \tau}=\frac{A_t}{A_s},\quad A_t:=A_{0t}=e^{\int_0^t \phi(\tau)\dd \tau}\\
B_{st}^2:=\xi \int_s^t A_{\tau t}^2 g^2(\tau)\dd \tau=\xi A^2_t\int_s^t\frac{g^2(\tau)}{A_\tau^2}\dd \tau, B_{st}\geq0
$$

>   证明：
>
>   若$\psi(x_t,t)\equiv 0, g(t)\equiv 0$，则$\dd x_t=\phi(t)x_t$，即$\dd \ln x_t=\phi(t)\dd t$，故

>   $$
>   \frac{x_t}{x_s}=A_{st}:=e^{\int_s^t \phi(\tau)\dd \tau},
>   $$
>   有性质

>   $$
>   \dd A_t=A_t \phi(t)\dd t.
>   $$
>   若$\psi(x_t,t)\not\equiv 0, g(t)\not\equiv 0$，则设辅助变量$y_t=\frac{x_t}{A_t}$，

>   $$
>   \dd x_t=y_t \dd A_t+A_t \dd y_t=y_t A_t\phi(t)\dd t+A_t\dd y_t=\phi(t)x_t\dd t+A_t \dd y_t,
>   $$
>   将上式和式$\eqref{eq:orig}$比较，得

>   $$
>   A_t \dd y_t= \psi( x_t,t)\dd t+g(t)\dd{ w_t},
>   $$
>   故

>   $$
>   \dd y_t=\frac 1 {A_t} [ \psi( x_t,t)\dd t+g(t)\dd{ w_t}],\\
>   \frac{x_t}{A_t}-\frac{x_s}{A_s}=y_t-y_s=\int_s^t \frac 1 {A_\tau} [ \psi( x_\tau,\tau)\dd \tau+g(\tau)\dd{ w_\tau}],\\
>   x_t=\frac{A_t}{A_s}x_s+\int_s^t  \frac {A_t} {A_\tau} [ \psi( x_\tau,\tau)\dd \tau+g(\tau)\dd{ w_\tau}],\\
>   x_t=A_{st} x_s+\int_s^t A_{\tau t} \psi(x_\tau,\tau)\dd \tau+\sqrt {\xi \int_s^t A_{\tau t}^2 g^2(\tau)\dd w_\tau} z, z\sim N(0,I)
>   $$
>   证毕

其中，

*   $A_{st}, B_{st}$：在$\phi(t),g(t)$不复杂时，可以解析算出该积分；
*   $\int_s^t A_{\tau t} \psi(x_\tau,\tau)\dd \tau$：在$\psi$是神经网络时，无法解析算出该积分，需要对$\psi$在$s$处关于$\tau-s$进行$k-1$阶泰勒展开，然后再积分。这样就得到了式$\eqref{eq:sol}$关于$s-t$的$k$阶近似。

## SDE/ODE-DPM-solver: 用$\epsilon_\theta$估计$z_t$

若扩散过程的正向SDE为
$$
\dd x_t=f(t)x_t\dd t+g(t)\dd w_t,
$$
则反向扩散可以使用其逆向SDE和ODE（如下）去采样，
$$
\begin{align}
逆向SDE&& & \dd x_t=[f(t)x_t-g^2(t)\nabla\log q(x_t)]\dd t+g(t)\dd {\bar w_t},\\
ODE & &&\dd x_t=[f(t)x_t-\frac 1 2g^2(t)\nabla\log q(x_t)]\dd t
\end{align}
$$
用神经网络$\epsilon_\theta$估计$z_t$，有$\nabla\log q(x_t)=-\frac{1}{b_t}\epsilon_\theta(x_t,t)$，代入上式，**得到：**
$$
\begin{align}
逆向SDE&& & \dd x_t=[f(t)x_t+\frac{g^2(t)}{b_t}\epsilon_\theta(x_t,t)]\dd t+g(t)\dd {\bar w_t},\\
ODE & &&\dd x_t=[f(t)x_t+\frac{g^2(t)}{b_t}\epsilon_\theta(x_t,t)]\dd t
\end{align}
$$
对比式$\eqref{eq:orig}$，得$\phi(t)=f(t),\psi(x_t,t)=\frac{g^2(t)}{b_t}\epsilon_\theta(x_t,t)$。

故式$\eqref{eq:sol}$中的系数为，
$$
A_{st}=\exp\int_s^t f(\tau)\dd \tau=:a_{st}=\frac{a_t}{a_s},\quad a_{t}:=a_{0t}\\
\int_s^t A_{\tau t}^2 g^2(\tau)\dd \tau=a_t^2\int_s^t \frac{g^2(\tau)}{a_\tau^2}\dd \tau=a_t^2(c_t^2-c_s^2)\\
其中c_t^2:=\int_0^t \frac{g^2(\tau)}{a_\tau^2}\dd \tau
$$
故**精确的积分形式解**为，
$$
\begin{align}
\text{SDE-DPM-solver}&&& x_t=a_{st} x_s+\int_s^t a_{\tau t} \frac{g^2(\tau)}{b_\tau}\epsilon_\theta(x_\tau,\tau)\dd \tau+a_t\sqrt{c_s^2-c_t^2} z, z\sim N(0,I)\\
\text{(ODE-)DPM-solver} &&& x_t=a_{st} x_s+\frac 1 2\int_s^t a_{\tau t} \frac{g^2(\tau)}{b_\tau}\epsilon_\theta(x_\tau,\tau)\dd \tau
\end{align}
$$
对上式使用关于$s-t$的1阶近似，$\psi(x_\tau,\tau)\approx \psi(x_s,s), \forall \tau\in[s,t]$，并使用性质$b_t=a_tc_t$，则其中的积分为
$$
\int_s^t a_{\tau t} \frac{g^2(\tau)}{b_\tau}\epsilon_\theta(x_\tau,\tau)\dd \tau=
a_t \int_s^t  \frac{g^2(\tau)}{a^2_\tau c_\tau}\dd \tau \epsilon_\theta(x_s,s)\\=a_t\int_s^t \frac{\dd (c_\tau^2)}{\sqrt{c_\tau^2}}=2a_t(\sqrt{c_t^2}-\sqrt{c_s^2})=2a_t(c_t-c_s)
$$
故**1阶近似后的积分形式解**为，
$$
\begin{align}
\text{SDE-DPM-solver-1}&&& x_t=a_{st} x_s+ 2a_t(c_t-c_s) \epsilon_\theta(x_s,s)+a_t\sqrt{c_s^2-c_t^2} z, z\sim N(0,I),& DDPM关于s-t的近似\label{eq:diff-eps-sde}\\
\text{(ODE-)DPM-solver-1}&&& x_t=a_{st} x_s+ a_t(c_t-c_s) \epsilon_\theta(x_s,s), &  DDIM/\text{DPM-solver-1}\label{eq:diff-eps-ode}
\end{align}
$$

* $\eqref{eq:diff-eps-sde}$是DDPM关于$s-t$的1阶近似。证明见$\eqref{eq:diff-sde-x}$后续的说明。
* 对式$\eqref{eq:diff-eps-ode}$变形
$$
x_t=a_{st} x_s+ a_t(c_t-c_s) \epsilon_\theta(x_s,s)\\
=\frac{a_{t}}{a_s} x_s+ a_t(\frac {b_t}{a_t}-\frac{b_s}{a_s}) \epsilon_\theta(x_s,s)\\
=\frac{a_{t}}{a_s} x_s-(\frac{a_t}{a_s}b_s-b_t) \epsilon_\theta(x_s,s)
$$
它和DPM-solver-1的式$\eqref{eq:dpm-sol-1}和$DDIM的式$\eqref{eq:ddim-st}$一致。故式$\eqref{eq:diff-eps-ode}$就是 **DPM-solver-1， 就是 DDIM**。

## SDE/ODE-DPM-solver++: 用$x_\theta$估计$x_0$​
...
\end{align}
$$
用神经网络$x_\theta$估计$x_0$，有$\nabla\log q(x_t)=-\frac{x_t-a_t x_\theta(x_t,t)}{b_t^2}$，代入上式，得到：
$$
\begin{align}
逆向SDE&& & \dd x_t=[f(t)x_t+\frac{g^2(t)}{b_t^2}\big(x_t-a_t x_\theta(x_t,t)\big)]\dd t+g(t)\dd {\bar w_t},\\
ODE & &&\dd x_t=[f(t)x_t+ \frac{g^2(t)}{2 b_t^2}\big(x_t-a_t x_\theta(x_t,t)\big)]\dd t
\end{align}
$$
化简得到，
$$
\begin{align}
逆向SDE&& & \dd x_t=[f(t)+\frac{g^2(t)}{b_t^2}]x_t\dd t -\frac{a_t g^2(t)}{b_t^2}x_\theta(x_t,t)\dd t+g(t)\dd {\bar w_t},\\
ODE & && \dd x_t=[f(t)+\frac{g^2(t)}{2b_t^2}]x_t\dd t -\frac{a_t g^2(t)}{2b_t^2}x_\theta(x_t,t)\dd t
\end{align}
$$

*   将逆向SDE和式$\eqref{eq:orig}$对比，得$\phi(t)=f(t)+\frac{g^2(t)}{b_t^2},\psi(x_t,t)=-\frac{a_t g^2(t)}{b_t^2}x_\theta(x_t,t)$​。故式$\eqref{eq:sol}$中的系数为，

$$
A_{st}=\exp\int_s^t(f(\tau)+\frac{g^2(\tau)}{b_\tau^2})\dd \tau
=\exp\int_s^t(f(\tau)+\frac{g^2(\tau)}{a_\tau^2 c_\tau^2})\dd \tau\\
=\frac{a_t}{a_s}\exp\int_s^t\frac{\dd (c_\tau^2)}{ c_\tau^2}=\frac{a_t}{a_s}\exp\ln\frac{c_t^2}{c_s^2}=\frac{a_tc_t^2}{a_sc_s^2}\\

\int_s^t A_{\tau t}^2 g^2(\tau)\dd \tau=a_t^2c^4_t\int_s^t \frac{g^2(\tau)}{a_\tau^2c_\tau^4}\dd \tau=a_t^2c^4_t\int_s^t \frac{\dd (c_\tau^2)}{(c_\tau^2)^2}\\
=a_t^2c^4_t(\frac{1}{c_s^2}-\frac{1}{c_t^2})=a_t^2c^2_t(\frac{c_t^2}{c_s^2}-1)\\

\int_s^t A_{\tau t} \psi(x_\tau,\tau)\dd \tau=
\int_s^t \frac{a_tc_t^2}{a_\tau c_\tau^2} \frac{a_\tau g^2(\tau)}{b^2_\tau}x_\theta(x_\tau,\tau)\dd \tau\\
= \int_s^t \frac{a_tc_t^2  g^2(\tau)}{a^2_\tau c_\tau^4}x_\theta(x_\tau,\tau)\dd \tau
$$

*   将ODE和式$\eqref{eq:orig}$对比，得$\phi(t)=f(t)+\frac{g^2(t)}{2 b_t^2},\psi(x_t,t)=-\frac{a_t g^2(t)}{2 b_t^2}x_\theta(x_t,t)$。故式$\eqref{eq:sol}$​中的系数为，
    $$
    A_{st}=\exp\int_s^t(f(\tau)+\frac{g^2(\tau)}{2b_\tau^2})\dd \tau
    =\exp\int_s^t(f(\tau)+\frac{g^2(\tau)}{2a_\tau^2 c_\tau^2})\dd \tau\\
    =\frac{a_t}{a_s}\exp\int_s^t\frac{\dd (c_\tau^2)}{2 c_\tau^2}=\frac{a_t}{a_s}\exp[\frac 1 2\ln\frac{c_t^2}{c_s^2}]=\frac{a_tc_t}{a_sc_s}\\

    \int_s^t A_{\tau t} \psi(x_\tau,\tau)\dd \tau=
    \int_s^t \frac{a_tc_t}{a_\tau c_\tau} \frac{a_\tau g^2(\tau)}{2b^2_\tau}x_\theta(x_\tau,\tau)\dd \tau\\
    = \int_s^t \frac{a_tc_t  g^2(\tau)}{2a^2_\tau c_\tau^3}x_\theta(x_\tau,\tau)\dd \tau
    $$

故**精确的积分形式解**为，
$$
\begin{align}
\text{SDE-DPM-solver++}&&& x_t=\frac{a_tc_t^2}{a_sc_s^2} x_s-\int_s^t \frac{a_tc_t^2  g^2(\tau)}{a^2_\tau c_\tau^4}x_\theta(x_\tau,\tau)\dd \tau+a_tc_t\sqrt{1-\frac{c_t^2}{c_s^2}} z, z\sim N(0,I)\\
\text{(ODE-)DPM-solver++} &&& x_t=\frac{a_tc_t}{a_sc_s} x_s-\frac 1 2\int_s^t \frac{a_tc_t  g^2(\tau)}{a^2_\tau c_\tau^3}x_\theta(x_\tau,\tau)\dd \tau
\end{align}
$$
对上式使用关于$s-t$的1阶近似，$\psi(x_\tau,\tau)\approx \psi(x_s,s), \forall \tau\in[s,t]$，并使用性质$b_t=a_tc_t$，则其中的积分为
$$
\begin{align}
逆向SDE: & -\int_s^t \frac{a_tc_t^2  g^2(\tau)}{a^2_\tau c_\tau^4}x_\theta(x_\tau,\tau)\dd \tau
=-a_tc_t^2 \int_s^t \frac{ g^2(\tau)}{a^2_\tau c_\tau^4}\dd \tau x_\theta(x_s,s)\\
& =-a_tc_t^2 \int_s^t  \frac{ \dd (c_\tau^2)}{ (c_\tau^2)^2} x_\theta(x_s,s)=a_tc_t^2 (\frac 1 {c_t^2}-\frac 1 {c_s^2})x_\theta(x_s,s)=a_t  (1-\frac {c_t^2} {c_s^2})x_\theta(x_s,s)\\

ODE:& -\frac 1 2\int_s^t \frac{a_tc_t  g^2(\tau)}{a^2_\tau c_\tau^3}x_\theta(x_\tau,\tau)\dd \tau=-\frac {a_tc_t} 2\int_s^t \frac{ g^2(\tau)}{a^2_\tau c_\tau^3}\dd \tau x_\theta(x_s,s)\\
& =-\frac {a_tc_t} 2\int_s^t \frac{\dd({c_\tau^2})}{(c_\tau^2)^{\frac 3 2}}\dd \tau x_\theta(x_s,s)=-\frac {a_tc_t} 2 2(\frac {1} {\sqrt{c_s^2}}-\frac {1} {\sqrt{c_t^2}}) x_\theta(x_s,s)\\
& =a_tc_t(\frac 1 {c_t}-\frac 1 {c_s}) x_\theta(x_s,s)=a_t (1-\frac{c_t}{c_s})x_\theta(x_s,s)
\end{align}
$$
故**1阶近似后的积分形式解**为，
$$
\begin{align}
 \text{SDE-DPM-solver++1}&&& x_t=\frac{a_tc_t^2}{a_sc_s^2} x_s+ a_t  (1-\frac {c_t^2} {c_s^2}) x_\theta(x_s,s)+a_tc_t\sqrt{1-\frac{c_t^2}{c_s^2}}  z, z\sim N(0,I) &DDPM \label{eq:diff-sde-x}\\
 \text{(ODE-)DPM-solver++1} &&& x_t=\frac{a_tc_t}{a_sc_s}  x_s+ a_t (1-\frac{c_t}{c_s}) x_\theta(x_s,s)&  DDIM/\text{DPM-solver-1}\label{eq:diff-ode-x}
\end{align}
$$
式$\eqref{eq:diff-sde-x}$化简一下，即下式，和DDPM 式$\eqref{eq:ddpm-x0}$一样。故式$\eqref{eq:diff-sde-x}$即DDPM。
$$
x_t = \frac{1}{c_s^2}(a_t(c_s^2-c_t^2)x_\theta(x_s,s)+\frac{a_t}{a_s}c_t^2x_s) + \sqrt{\frac{a_t^2c_t^2(c_s^2-c_t^2)}{c_s^2}}z,z\sim N(0,I)
$$
若使用下文的$\epsilon_\theta$网络，来估计$x_0$, 即$x_\theta(x_s,s)=\frac{x_s-b_s \epsilon_\theta(x_s,s)}{a_s}$，

*   将其带入上式$\eqref{eq:diff-ode-x}$，得

$$
x_t=\frac{a_t c_t}{a_s c_s} x_s+ a_t (1-\frac{c_t}{c_s}) x_\theta(x_s,s)\\
=\frac{a_t c_t}{a_s c_s} x_s+ \frac{a_t}{a_s} (1-\frac{c_t}{c_s}) (x_s-b_s \epsilon_\theta(x_s,s))\\
=\frac{a_t}{a_s}x_s-\frac{a_t}{a_s} (1-\frac{c_t}{c_s}) b_s \epsilon_\theta(x_s,s)\\
=\frac{a_t}{a_s}x_s-\frac{a_t}{a_s} (1-\frac{c_t}{c_s}) a_s c_s \epsilon_\theta(x_s,s)\\
=\frac{a_t}{a_s}x_s+a_t (c_t- c_s) \epsilon_\theta(x_s,s)
$$

​	和式$\eqref{eq:diff-eps-ode}$一样。故若使用下文的$\epsilon_\theta$网络来估计$x_0$，则式$\eqref{eq:diff-ode-x}$就是**DPM-solver-1**， 也是**DDIM**。

*   将其带入上式$\eqref{eq:diff-sde-x}$，得
    $$
    \begin{align}
    x_t&=\frac{a_tc_t^2}{a_sc_s^2} x_s+ \frac{a_t}{a_s}  (1-\frac {c_t^2} {c_s^2}) (x_s-b_s \epsilon_\theta(x_s,s))+a_tc_t\sqrt{1-\frac{c_t^2}{c_s^2}}  z\\
    &=\frac{a_t}{a_s}x_s-a_tc_s(1-\frac {c_t^2} {c_s^2}) \epsilon_\theta(x_s,s)+a_tc_s\frac{c_t}{c_s}\sqrt{1-\frac{c_t^2}{c_s^2}}  z
    \end{align}
    $$
    由于$\frac{c_s-c_t}{c_s}>0,\approx 0$，故设 $x=1-\frac{c_t}{c_s}$，有$o(x)=o(s-t)$
    $$
    \begin{align}
    x_t&=\frac{a_t}{a_s}x_s-a_tc_s (2-x)x\epsilon_\theta(x_s,s)+a_tc_s (1-x)\sqrt{(2-x)x}  z\\
    &=\frac{a_t}{a_s}x_s-a_tc_s 2x\epsilon_\theta(x_s,s)+O(x^2)+a_tc_s\sqrt{(2-x)x} z+O(x^{3/2})\\
    &=\frac{a_t}{a_s}x_s-2a_tc_s x\epsilon_\theta(x_s,s)+a_tc_s\sqrt{(2-x)x} z+o(s-t)\\
    & \approx \frac{a_t}{a_s}x_s-2a_tc_s (1-\frac{c_t}{c_s})\epsilon_\theta(x_s,s)+a_tc_s\sqrt{1-\frac{c^2_t}{c^2_s}} z\\
    & =\frac{a_t}{a_s}x_s+2a_t (c_t-c_s)\epsilon_\theta(x_s,s)+a_t\sqrt{c_s^2-c_t^2} z
    \end{align}
    $$
    和式$\eqref{eq:diff-eps-sde}$一样。故若使用下文的$\epsilon_\theta$网络来估计$x_0$，式$\eqref{eq:diff-sde-x}$（DDPM）就是$\eqref{eq:diff-eps-sde}$的关于$s-t$的一阶近似。

# DPM-Solver

一种**在DDIM的设定**下的，DPM（Diffusion Probabilistic Model, 扩散概率模型）反向传播的高阶采样加速方法。DDIM是1阶的DPM-Solver。
...
>
> $$
y_t=y_s+\int_s^te^{-F|^\tau_s}G(x_\tau,\tau)\dd \tau
> $$
>
> 再带入$y_t$的定义，得
...
> $$
> x_t=e^{F|^t_s}x_s+\int_s^te^{F|^t_\tau}G(x_\tau,\tau)\dd \tau. \ \ \ \  \square
> $$
...
带入上式求积分，可得DPM-Solver的**k阶迭代公式**，记作**DPM-solver-k**，是关于$h$的$k$阶精度的算法，使用$k-1$阶导，误差为$k+1$阶：
...
即 x_t=\frac{a_t}{a_s}x_s- (\frac{a_{t}}{a_{s}}b_{s}-b_t)\epsilon_\theta(x_s,s)\label{eq:dpm-sol-1}
$$
...
> $$
> \begin{align}
> x_t&=\frac{a_t}{a_s}x_s-b_t (\frac{a_{t}}{b_{t}} \frac{b_{s}}{a_{s}}-1)\epsilon_\theta(x_s,s)\\
> &=\frac{a_t}{a_s}x_s- (\frac{a_{t}}{a_{s}}b_{s}-b_t)\epsilon_\theta(x_s,s)\\
> 即
...
$$
\begin{align}
x_\tau &=\kappa_{t\tau} x_t+(a_\tau-a_t \kappa_{t\tau})x_0+\sigma_t z, z\sim N(0,I),\kappa_{t\tau}=\frac{\sqrt{b^2_{\tau}-\sigma_t^2}}{b_t}\\
&=\kappa_{t\tau} x_t+(a_\tau-a_t \kappa_{t\tau})(\mu_{0|t}(x_t)+\sigma_{0|t}z_{0|t})+\sigma_t z, z_{0|t}\sim N(0,I)\\
&=\kappa_{t\tau} x_t+(a_\tau-a_t \kappa_{t\tau})\mu_{0|t}(x_t)+\tilde\sigma_t \tilde z,\tilde z\sim N(0,I)
\end{align}
$$
...
$$
\tilde\sigma_t^2=(a_\tau-a_t\kappa_{t\tau})^2\sigma_{0|t}^2+\sigma_t^2
$$

多出了$(a_\tau-a_t\kappa_{t\tau})^2\sigma_{0|t}^2$这项。因此，即使像DDIM取$\sigma_t=0$，$q(x_\tau |x_t)$也有噪声项。
...
$$
\mathbb{E}_{x_t\sim q(x_t)}\mathbb{E}_{x_0\sim q(x_0|x_t)} -\log N(x_0|\mu_t(x_t),\sigma_t^2I)\\
=\mathbb{E}_{x_t\sim q(x_t)}\mathbb{E}_{x_0\sim q(x_0|x_t)} \frac{\Vert x_0-\mu_t(x_t) \Vert^2}{2\sigma_{0|t}^2}+\frac{d}{2}\log \sigma_{0|t}^2+\frac{d}{2}\log2\pi
$$
故，上式取得最小值时，
$$
\sigma_{0|t}^2=\frac{1}{d}\mathbb{E}_{x_0\sim q(x_0)}\mathbb{E}_{x_t\sim q(x_t|x_0)}\Vert x_0-\mu_t(x_t) \Vert^2
$$
由于均值估计不准确，无法使用$\mu_t(x_t)=\mathbb{E}_{x_0\sim q(x_0|x_t)} x_0$。只能带入$x_0=\frac{x_t-b_tz_t}{a_t}$, $\mu_t(x_t)=\frac{x_t-b_t\epsilon_\theta(x_t,t)}{a_t}$​，得

#### 单值方差

$$
\sigma_{0|t}^2=\frac{b_t^2}{da_t^2}\mathbb{E}_{x_0\sim q(x_0),z_t\sim N(0,I)} \Vert z_t-\epsilon_\theta(a_tx_0+b_tz_t,t)\Vert^2
$$

#### 各维方差

$$
\boldsymbol \sigma_{0|t}^2=\frac{b_t^2}{a_t^2}\mathbb{E}_{x_0\sim q(x_0),z_t\sim N(0,I)} (z_t-\epsilon_\theta(a_tx_0+b_tz_t,t))^2
$$

### 条件方差

想在模型$\mu_t(x_t)$训练好后，固定它，再去训练一个网络预测条件方差，

#### 各维方差

训练一个网络$\boldsymbol \sigma_{0|t}^2(x_t)$预测条件方差，
$$
\boldsymbol \sigma_{0|t}^2(x_t)=\mathbb{E}_{x_0\sim q(x_0|x_t)} (x_0-\mu_t(x_t))^2=\boldsymbol \sigma_{0|t}^2=\frac{b_t^2}{a_t^2}\mathbb{E}_{x_0\sim q(x_0|x_t)} (z_t-\epsilon_\theta(x_t,t))^2, 其中z_t=\frac{x_t-a_t x_0}{b_t}
$$
由于$\mathbb{E}_x x=\arg\min_{\mu}\Vert x-\mu \Vert^2$，故
$$
\boldsymbol \sigma_{0|t}^2(x_t)=\frac{b_t^2}{a_t^2}\arg\min_{\boldsymbol g(x_t)}\mathbb{E}_{x_0\sim q(x_0|x_t)} \Vert (z_t-\epsilon_\theta(x_t,t))^2-\boldsymbol g(x_t) \Vert^2, \boldsymbol g(x_t)\in \R^d\\
故，
\boldsymbol \sigma_{0|t}^2函数=\frac{b_t^2}{a_t^2}\arg\min_{\boldsymbol g网络的参数}\mathbb{E}_{x_t\sim q(x_t)}\mathbb{E}_{x_0\sim q(x_0|x_t)} \Vert (z_t-\epsilon_\theta(x_t,t))^2-\boldsymbol g(x_t) \Vert^2\\
=\frac{b_t^2}{a_t^2}\arg\min_{\boldsymbol g网络的参数}\mathbb{E}_{x_0\sim q(x_0)}\mathbb{E}_{x_t\sim q(x_t|x_0)} \Vert (z_t-\epsilon_\theta(x_t,t))^2-\boldsymbol g(x_t) \Vert^2\\
=\frac{b_t^2}{a_t^2}\arg\min_{\boldsymbol g网络的参数}\mathbb{E}_{x_0\sim q(x_0)}\mathbb{E}_{z_t\sim N(0,I)} \Vert (z_t-\epsilon_\theta(a_tx_0+b_tz_t,t))^2-\boldsymbol g(x_t) \Vert^2
$$
最后一行即训练的$\boldsymbol \sigma_{0|t}^2(x_t)$网络的方法。

#### 单值方差

训练一个网络$\sigma_{0|t}^2(x_t)$预测条件方差，
$$
\sigma_{0|t}^2函数=\frac{b_t^2}{da_t^2}\arg\min_{ g网络的参数}\mathbb{E}_{x_0\sim q(x_0)}\mathbb{E}_{z_t\sim N(0,I)} \big\Vert \Vert z_t-\epsilon_\theta(a_tx_0+b_tz_t,t)\Vert^2- g(x_t) \big\Vert^2, g(x_t)\in \R
$$

### 两阶段训练

在[《Improved Denoising Diffusion Probabilistic Models》](https://papers.cool/arxiv/2006.11239)的可学习方差增加了训练难度。但Extended-Analytic-DPM做可学习方差，使用了两阶段训练，避免了这一问题。具体做法是，先训练预测均值的模型$\mu_t(x_t)$（即预测噪声的模型$\epsilon_\theta(x_t,t)$，此时使用DDPM的固定方差；训练好均值预测模型后，固定该模型，加入可学习方差模型，训练它。
