...
x_t&=a_tx_0+b_tz_t\\
&将上两式带入下式\\
x_{\tau}&=\kappa_{t\tau} x_t+\lambda_{t\tau}x_0+\sigma_\tau z
\end{align}
$$
...
b_{\tau}^2=\sigma_\tau^2+\kappa_{t\tau}^2 b^2
$$

两个方程，求解$\kappa_{t\tau},\lambda_{t\tau},\sigma_\t$三个变量，解有无穷个，可见$q(\boldsymbol x_{t-1}|\boldsymbol x_t,\boldsymbol x_0)$有很多种选择。故先将$\sigma_t$当作待定参数，去求余下两个变量。

$$
\kappa_{t\tau}=\frac{\sqrt{b^2_{\tau}-\sigma_\tau^2}}{b_t}\\
...
$$
x_\tau=\kappa_{t\tau} x_t+(a_\tau-a_t \kappa_{t\tau})x_0+\sigma_\tau z, z\sim N(0,I)\\
\kappa_{t\tau}=\frac{\sqrt{b^2_{\tau}-\sigma_\tau^2}}{b_t} \label{ddim-repara}
$$

* 当$\sigma_\tau=0$，**为DDIM**，上式变为

  $$
  x_\tau=\frac{b_\tau}{b_t}x_t+(a_\tau-a_t \frac{b_\tau}{b_t})x_0
  $$
* 当$\sigma_\tau=\tilde b'_t$，$\tau=t-1$，为DDPM，上式变为$\eqref{qxt-1-xt-x0}$，即
...
$$
\begin{align}
x_\tau &=\kappa_{t\tau} x_t +(a_\tau-a_t\kappa_{t\tau})\frac{1}{a_t}(x_t-b_t\epsilon_\theta(x_t,t))+\sigma_\tau z_t\\
&=\frac{a_\tau}{a_t}x_t-(\frac{a_\tau}{a_t}b_t-b_t\kappa_{t\tau})\epsilon_\theta(x_t,t)+\sigma_\tau z\\
&=\frac{a_\tau}{a_t}x_t-(\frac{a_\tau}{a_t}b_t-\sqrt{b^2_\tau-\sigma_t^2})\epsilon_\theta(x_t,t)+\sigma_\tau z\label{ddim-general}
\end{align}
$$
...
$$
x_\tau=\frac{\sqrt{\bar\alpha_{\tau}}}{\sqrt{\bar\alpha_t}}(x_t-\sqrt{1-\bar\alpha_t}\epsilon_\theta(x_t,t))+\sqrt{{1-\bar\alpha_{\tau}-\sigma_t^2}}\epsilon_\theta(x_t,t)+\sigma_\tau z,z\sim N(0,I)
$$

* 当$\sigma_\tau=0$，**即为DDIM**，$q(x_t|x_{t-1})$不满足马尔可夫性，式$\eqref{ddim-general}$变为
  ...
  * 若又$\tau=t-1$，则
  ...
  $$
* 当$\sigma_\tau=\tilde b_{t|\tau}:=\dfrac{b_\tau}{b_t}b_{t|\tau}$，为DDPM跳步采样，$q(x_t|x_{t-1})$满足马尔可夫性，式$\eqref{ddim-general}$变为
