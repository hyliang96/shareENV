...
$$
\begin{align}
(\epsilon_\theta \text{-)SDE-DPM-solver}&&& x_t=a_{st} x_s+\int_s^t a_{\tau t} \frac{g^2(\tau)}{b_\tau}\epsilon_\theta(x_\tau,\tau)\dd \tau+a_t\sqrt{c_s^2-c_t^2} z, z\sim N(0,I)\\
(\epsilon_\theta\text{-ODE-)DPM-solver} &&& x_t=a_{st} x_s+\frac 1 2\int_s^t a_{\tau t} \frac{g^2(\tau)}{b_\tau}\epsilon_\theta(x_\tau,\tau)\dd \tau
\end{align}
$$
...
$$
\begin{align}
(\epsilon_\theta\text{-)SDE-DPM-solver-1}&&& x_t=a_{st} x_s+ 2a_t(c_t-c_s) \epsilon_\theta(x_s,s)+a_t\sqrt{c_s^2-c_t^2} z, z\sim N(0,I),& DDPM关于s-t的近似\label{eq:diff-eps-sde}\\
(\epsilon_\theta\text{-ODE-)DPM-solver-1}&&& x_t=a_{st} x_s+ a_t(c_t-c_s) \epsilon_\theta(x_s,s), &  DDIM/\text{DPM-solver-1}\label{eq:diff-eps-ode}
\end{align}
$$
...
$$
\begin{align}
x_\theta \text{-SDE-DPM-solver}&&& x_t=\frac{a_tc_t^2}{a_sc_s^2} x_s-\int_s^t \frac{a_tc_t^2  g^2(\tau)}{a^2_\tau c_\tau^4}x_\theta(x_\tau,\tau)\dd \tau+a_tc_t\sqrt{1-\frac{c_t^2}{c_s^2}} z, z\sim N(0,I)\\
x_\theta \text{-(ODE-)DPM-solver} &&& x_t=\frac{a_tc_t}{a_sc_s} x_s-\frac 1 2\int_s^t \frac{a_tc_t  g^2(\tau)}{a^2_\tau c_\tau^3}x_\theta(x_\tau,\tau)\dd \tau
\end{align}
$$
对上式使用关于$s-t$的1阶近似，$\psi(x_\tau,\tau)\approx \psi(x_s,s), \forall \tau\in[s,t]$，并使用性质$b_t=a_tc_t$，则其中的积分为
$$
...
$$
\begin{align}
x_\theta \text{-SDE-DPM-solver++1}&&& x_t=\frac{a_tc_t^2}{a_sc_s^2} x_s+ a_t  (1-\frac {c_t^2} {c_s^2}) x_\theta(x_s,s)+a_tc_t\sqrt{1-\frac{c_t^2}{c_s^2}}  z, z\sim N(0,I) &DDPM \label{eq:diff-sde-x}\\
x_\theta \text{-(ODE-)DPM-solver++1} &&& x_t=\frac{a_tc_t}{a_sc_s}  x_s+ a_t (1-\frac{c_t}{c_s}) x_\theta(x_s,s)&  DDIM/\text{DPM-solver-1}\label{eq:diff-ode-x}
\end{align}
$$
