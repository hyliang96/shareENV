...
$$
\begin{gather}
\operatorname*{\arg\min}_{v\in \mathcal U,\rho\in \mathcal P}  \int_0^1\int_{\R^n}\left[ \frac 1 {2} \Vert v(t,x)-\bar v(t,x)\Vert^2+\frac{g^2(t)}{8}\Vert\nabla\log\frac{\rho(t,x)}{\bar\rho(t,x)}\Vert^2\right]\rho(t,x)\dd x\dd t\label{gen-v-loss}\\
FPK方程：\frac{\partial \rho}{\partial t}+\nabla\cdot(v\rho )=0\\
\rho(0,x)=\rho_0(x),\rho(1,y)=\rho_1(y)
\end{gather}
$$
...
$P$测度下：
$$
\dd X_t= [f(t,X_t)+u(t,X_t)]\dd t+g \dd W_t,\\
W_t在概率测度P下是标准布朗运动\\
...
$Q$测度下：
$$
\dd X_t= u(t,X_t)\dd t+g \dd {\bar W_t},\\
\bar W_t在概率测度Q=\bar P下是标准布朗运动\\
