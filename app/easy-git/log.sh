...
EOF
    ...
    alias _glg="git log --graph --abbrev-commit --decorate=no --date=format:'%Y-%m-%d' --pretty=format:'%C(yellow)%h%Creset%C(auto)%d%Creset %Cgreen%cd %cr %C(bold blue)%an%Creset %C(bold gray)%s%C(reset)' $cmd"
    alias _glg_table="git log --graph --abbrev-commit --decorate=no --date=format:'%Y-%m-%d' --format=format:'%C(yellow)%h%C(reset) %C(auto)%d%C(reset) %C(green)%>|(50)%ad %ar %C(reset)  %C(blue)%<(16,trunc)%an%C(reset) %C(bold gray)%>|(1)%s%C(reset)' $cmd"
...
alias ghs="git reflog --abbrev-commit --pretty=format:'%C(yellow)%h%C(reset)%C(yellow) - %gd%C(reset)%C(auto)%d%Creset %C(green)%cr%C(reset) %C(bold blue)%an%C(reset) %C(bold white)%gs%C(reset) %C(bold gray)%s%C(reset)'"
alias ghs-no-action="git reflog --abbrev-commit --pretty=format:'%C(yellow)%h%C(reset)%C(yellow) - %gd%C(reset)%C(auto)%d%C(reset) %C(green)%cr%C(reset) %C(bold blue)%an%C(reset) %C(bold gray)%s%C(reset)'"
